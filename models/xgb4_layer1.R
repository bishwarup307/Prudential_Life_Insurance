...
evalMatrix <- data.frame(Fold = numeric(), Id = numeric(), ground_truth = numeric(), xgb4_preds = numeric())
...
for(i in 1:10) {
  ...
  valid <- data.frame(Fold = rep(i, nrow(validationSet)), Id = validationSet$Id, ground_truth = validationSet$Response, xgb4_preds = preds)
...
write_csv(evalMatrix, "./Ensemble2/xgb4_eval.csv")
write_csv(testMatrix, "./Ensemble2/xgb4_test.csv")