...
# scale & log transformation of features
...
evalMatrix <- data.frame(Fold = numeric(), Id = numeric(), ground_truth = numeric(), svr4_preds = numeric())
...
for(i in 1:10) {
  ...
  valid <- data.frame(Fold = rep(i, nrow(validationSet)), Id = validationSet$Id, ground_truth = validationSet$Response, svr4_preds = preds)
  evalMatrix <- rbind(evalMatrix, valid)
  
}
# generate test meta-features 
# train on all training instances
svp_full <- LiblineaR(as.matrix(ptr[, feature.names]), 
                      ptr$Response, 
                      type =  11, 
                      cost = 10000, 
                      svr_eps = 1e-7, 
...
tpreds <- as.numeric(unlist(predict(svp_full, newx = as.matrix(pte[, feature.names]))))
testMatrix$svr4_preds <- tpreds

# save meta to disk
write_csv(evalMatrix, "./Ensemble2/svr4_eval.csv")
write_csv(testMatrix, "./Ensemble2/svr4_test.csv")
