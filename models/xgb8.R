...
evalMatrix <- data.frame(Fold = numeric(), Id = numeric(), ground_truth = numeric(), xgb8_preds = numeric())
...
for(i in 1:10) {
  ...
  valid <- data.frame(Fold = rep(i, nrow(validationSet)), Id = validationSet$Id, ground_truth = validationSet$Response, xgb8_preds = preds)
...
testMatrix$xgb8_preds <- test_preds

write_csv(evalMatrix, "./Ensemble2/xgb8_eval.csv")
write_csv(testMatrix, "./Ensemble2/xgb8_test.csv")
