...
evalMatrix <- data.frame(Fold = numeric(), Id = numeric(), ground_truth = numeric(), knn_preds = numeric())
...
for(i in 1:10) {
  ...
  kneigh <- knnreg(x = trainingSet[, feature.names],
                    y = trainingSet$Response,
                    k = 3)
  
  
  preds <- predict(kneigh, validationSet[, feature.names])
  
  valid <- data.frame(Fold = rep(i, nrow(validationSet)), Id = validationSet$Id, ground_truth = validationSet$Response, knn_preds = preds)
  evalMatrix <- rbind(evalMatrix, valid)
  
}

kneigh_F <- knnreg(x = ptr[, feature.names],
                 y = ptr$Response,
                 k = 3)

tpreds <- predict(kneigh_F, pte[, feature.names])
testMatrix$knn_preds <- tpreds

write_csv(evalMatrix, "./Ensemble2/knn1_eval.csv")
write_csv(testMatrix, "./Ensemble2/knn1_test.csv")
