...
evalMatrix <- data.frame(Fold = numeric(), Id = numeric(), ground_truth = numeric(), svr3_preds = numeric())
...
for(i in 1:10) {
  ...
  valid <- data.frame(Fold = rep(i, nrow(validationSet)), Id = validationSet$Id, ground_truth = validationSet$Response, svr3_preds = preds)
...
testMatrix$svr3_preds <- tpreds
# save meta to disk
write_csv(evalMatrix, "./Ensemble2/svr3_eval.csv")
write_csv(testMatrix, "./Ensemble2/svr3_test.csv")
