...
evalMatrix <- data.frame(Fold = numeric(), Id = numeric(), ground_truth = numeric(), gnet2_preds = numeric())
...
for(i in 1:10) {
  ...
  gnet <- glmnet(x = data.matrix(trainingSet[, feature.names]),
  ...
  preds <- predict(gnet, newx = data.matrix(validationSet[, feature.names]), s = 0.000001, type = "response")
  
  valid <- data.frame(Fold = rep(i, nrow(validationSet)), Id = validationSet$Id, ground_truth = validationSet$Response, gnet2_preds = preds)
  evalMatrix <- rbind(evalMatrix, valid)
  
}

gnet_full <- glmnet(x = data.matrix(ptr[, feature.names]),
               y = ptr$Response,
               lambda.min.ratio = 1e-9,
               family = "gaussian",
               nlambda = 500,
...
tpreds <- as.numeric(predict(gnet, newx = data.matrix(pte[, feature.names]), s = 0.000001, type = "response"))
testMatrix$gnet2_preds <- tpreds

write_csv(evalMatrix, "./Ensemble2/gnet2_eval.csv")
write_csv(testMatrix, "./Ensemble2/gnet2_test.csv")
